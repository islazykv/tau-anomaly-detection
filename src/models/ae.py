...
class Autoencoder(L.LightningModule):
    """Symmetric autoencoder for unsupervised anomaly detection.

    Trained on background-only events. Signal events are expected to produce
    higher reconstruction error.

    Args:
        cfg: Typed AE model configuration.
    ...
    def _compute_loss(self, x: Tensor, x_hat: Tensor, weights: Tensor) -> Tensor:
        ...
        Uses ``reduction="none"`` so that per-event weights can be applied:
        ``(loss.mean(dim=1) * w).mean()``.
        """
...
def _build_stack(
    ...
    *,
    ...
    """Build a fully-connected stack with activation and dropout between layers."""
...
def _get_loss_fn(name: str) -> nn.Module:
    """Return a loss function with ``reduction='none'``."""
    if name == "mse":
...
def _build_scheduler(
    ...
    """Build LR scheduler config for Lightning, or None if disabled."""
